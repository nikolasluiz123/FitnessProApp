...
@Dao
abstract class HealthConnectSleepSessionDAO : IntegratedMaintenanceDAO<HealthConnectSleepSession>() {

    @Query("select * from health_connect_sleep_session where id = :id")
    abstract suspend fun findById(id: String): HealthConnectSleepSession?
    ...
    suspend fun getExportationData(pageInfos: ExportPageInfos, personId: String): List<HealthConnectSleepSession> {
        ...
        val from = StringJoiner(QR_NL).apply {
            add(" from health_connect_sleep_session session ")
        }

        val where = StringJoiner(QR_NL).apply {
            add(" where session.transmission_state = '${EnumTransmissionState.PENDING.name}' ")
            // Subquery para garantir que estamos exportando apenas sessões
            // que estão ligadas a uma execução de exercício relevante para a pessoa.
            add(" and exists ( ")
    ...
    @RawQuery
    abstract suspend fun executeQueryExportationData(query: SupportSQLiteQuery): List<HealthConnectSleepSession>
}