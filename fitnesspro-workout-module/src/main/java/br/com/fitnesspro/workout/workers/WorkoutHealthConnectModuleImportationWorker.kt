...
@HiltWorker
class WorkoutHealthConnectModuleImportationWorker @AssistedInject constructor(
    ...
    override suspend fun onImport(serviceToken: String, lastUpdateDate: LocalDateTime?) {
        entryPoint.getHealthConnectModuleImportationRepository().import(serviceToken, lastUpdateDate)
    }

    override fun getModule() = EnumImportationModule.HEALTH_CONNECT

    override fun getClazz() = javaClass

    override fun getOneTimeWorkRequestBuilder(): OneTimeWorkRequest.Builder {
        return OneTimeWorkRequestBuilder<WorkoutHealthConnectModuleImportationWorker>()
    }
}