...
fun LineChartContainer(
    ...
    BoxWithConstraints(modifier = modifier) {
        ...
        val actualChartAreaHeightDp = (this.maxHeight - footerTotalHeightDp).coerceAtLeast(0.dp)
            ...
            Box(
                ...
                Canvas(modifier = Modifier.matchParentSize()) {
                    ...
                    drawYAxisGridLines(
                        ...
                        maxValue = maxValue,
                        zeroY = newZeroY,
                        chartHeight = actualChartAreaHeightDp
                    )

                    if (backgroundStyle.showXAxisLabels) {
                        val xAxisLabelPaint = backgroundStyle.xAxisLabelStyle.getTextPaint(drawContext.density)
                        
                        val slotWidthPx = if (backgroundStyle.enableHorizontalScroll) {
                            actualSlotWidthDp.toPx()
                        } else {
                             if (state.entries.isNotEmpty()) canvasWidth / state.entries.size else 0f
                        }
                        ...
                        state.entries.forEachIndexed { index, entry ->
                            ...
                            val xCenter = (index + 0.5f) * slotWidthPx
                ...
                Box(
                    ...
                    content(actualChartAreaHeightDp, totalChartWidthDp, actualSlotWidthDp)
                }
            }
        }
...
private fun calculateHorizontalMetrics(style: ChartContainerStyle, entryCount: Int, viewportWidth: Dp): Triple<Dp, Dp, Float> {
    val (totalChartWidthDp: Dp, actualSlotWidthDp: Dp) = if (style.enableHorizontalScroll && entryCount > 0) {
        val baseSlotWidth = style.scrollableBarWidth // Reutilizamos essa config por enquanto
...
private fun preCalculateFooterSizes(
    density: Density,
    state: LineChartState, // Tipo atualizado
    style: ChartContainerStyle,
