...
class GeneralModuleSyncWebClient(
    context: Context,
    private val service: GeneralModuleSyncService,
): FitnessProWebClient(context) {

    suspend fun export(token: String, dto: GeneralModuleSyncDTO): ExportationServiceResponse {
    ...
    suspend fun import(token: String, filter: CommonImportFilter, pageInfos: ImportPageInfos): ImportationServiceResponse<GeneralModuleSyncDTO> {
        return importationServiceErrorHandlingBlock(
            codeBlock = {
                ...
                ).getResponseBody(GeneralModuleSyncDTO::class.java)
            }
        )
    }
}