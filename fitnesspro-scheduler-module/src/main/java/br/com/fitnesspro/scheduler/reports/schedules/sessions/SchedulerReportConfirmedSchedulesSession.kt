...
class SchedulerReportConfirmedSchedulesSession(context: Context) : AbstractReportSession<SchedulerReportFilter>(context) {
    ...
    override suspend fun prepare(filter: SchedulerReportFilter) {
        ...
        this.title = context.getString(SchedulerRes.string.scheduler_report_confirmed_schedules_session_title)
        ...
        this.components = listOf(
            TableComponent<SchedulerReportFilter>(
                columns = listOf(
                    Column(
                        label = context.getString(SchedulerRes.string.scheduler_report_confirmed_schedules_session_column_name),
                        widthPercent = 0.4f
                    ),
                    Column(
                        label = context.getString(SchedulerRes.string.scheduler_report_confirmed_schedules_session_column_date),
                        widthPercent = 0.15f
                    ),
                    Column(
                        label = context.getString(SchedulerRes.string.scheduler_report_confirmed_schedules_session_column_time),
                        widthPercent = 0.2f
                    ),
                    Column(
                        label = context.getString(SchedulerRes.string.scheduler_report_confirmed_schedules_session_column_type),
                        widthPercent = 0.25f
                    )
                ),
