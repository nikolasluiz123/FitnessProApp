...
class SchedulerReportResumeSession(context: Context) : AbstractReportSession<SchedulerFilter>(context) {

    override suspend fun prepare(filter: SchedulerFilter) {
        super.prepare(filter)

        this.title = context.getString(SchedulerRes.string.scheduler_report_resume_session_title)
    }
